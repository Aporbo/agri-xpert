    ...
    <div className="p-6 max-w-6xl mx-auto space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">Track and analyze your soil health over time</p>
        </div>
        
        <div className="flex gap-2 bg-white p-1 rounded-lg shadow-sm border border-gray-200">
          <button
            onClick={() => setActiveTab('all')}
            className={`px-3 py-1 rounded-md text-sm ${activeTab === 'all' ? 'bg-emerald-100 text-emerald-700' : 'text-gray-600 hover:bg-gray-100'}`}
          >
            All Tests
          </button>
          <button
            onClick={() => setActiveTab('withRec')}
            className={`px-3 py-1 rounded-md text-sm ${activeTab === 'withRec' ? 'bg-emerald-100 text-emerald-700' : 'text-gray-600 hover:bg-gray-100'}`}
          >
            With Recommendations
          </button>
          <button
            onClick={() => setActiveTab('withoutRec')}
            className={`px-3 py-1 rounded-md text-sm ${activeTab === 'withoutRec' ? 'bg-emerald-100 text-emerald-700' : 'text-gray-600 hover:bg-gray-100'}`}
          >
            Pending Analysis
          </button>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8 text-center">
          <div className="mx-auto w-16 h-16 bg-emerald-100 rounded-full flex items-center justify-center mb-4">
            <FiInfo className="text-emerald-600 text-2xl" />
          </div>
                    ...
                    <div className="mt-4 space-y-3 animate-fadeIn">
                      ...
                      {recommendations[test._id] ? (
                        <div className="mt-3 bg-green-50 border border-green-200 rounded-lg p-3">
                          ...
                          <button
                            className="mt-3 w-full flex items-center justify-center gap-2 px-3 py-2 bg-emerald-600 text-white text-sm rounded-md hover:bg-emerald-700 transition"
                            onClick={(e) => {
                              e.stopPropagation();
